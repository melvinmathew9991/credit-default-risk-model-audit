...
for c in ["industry", "work_experience", "pincode", "role"]:
    m = enc.te.mapping[c]
    print(
        f"{c:<18} mapping rows={len(m):>6,}  distinct values={m.round(6).nunique():>3}  "
...
print(
    f"identical min? {np.isclose(mi.min(), mw.min())}   identical max? {np.isclose(mi.max(), mw.max())}"
)
...
for name, col, m in [("industry", "industry", mi), ("work_experience", "work_experience", mw)]:
    ...
    for idx in [m.idxmin(), m.idxmax()]:
        ...
        print(
            f"   encoded={m[idx]:.6f}  rows={n:>7,}  observed_default_rate={label_mean:.6f}  value={shown}"
        )
