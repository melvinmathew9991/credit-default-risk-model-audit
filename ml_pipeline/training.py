...
# Train lgb
def train_lgb(
    train, val, lgbm_params, num_boost_round=20000, early_stopping_rounds=50, non_feature_cols=None
):
    """
    ...
    logger.info(
        "training on %d features, %d rows (val %d rows)", len(feature_cols), len(train), len(val)
    )
    ...
    if clf.best_iteration >= num_boost_round:
        logger.warning(
            "early stopping never triggered - the model may be "
