...
def main():
    ...
    rows = []
    grid = [(5000, 1), (1000, 1), (500, 10), (200, 10), (100, 10), (50, 10), (20, 5), (10, 2)]
    ...
    for clean in (False, True):
        ...
        print(f"\n{'=' * 100}\n{tag}\n{'=' * 100}")
        print(
            f"{'min_samples_leaf':>17}{'smoothing':>11}{'trees':>8}"
        ...
        for msl, sm in grid:
            ...
            rows.append(r)
            print(
                f"{msl:>17}{sm:>11}{r['trees']:>8}{r['train_auc']:>11.4f}"
    ...
    for tag, g in res.groupby("config", sort=False):
        ...
        print(
            f"{tag:<26} best val_auc={b.val_auc:.4f} (gini {b.val_gini:.4f}) "
    ...
    print(
        f"  with RESTRICTED    ({with_r['n_features']:>2} features): val_auc={with_r['val_auc']:.4f}  gini={with_r['val_gini']:.4f}"
    )
    print(
        f"  without RESTRICTED ({without_r['n_features']:>2} features): val_auc={without_r['val_auc']:.4f}  gini={without_r['val_gini']:.4f}"
    )
