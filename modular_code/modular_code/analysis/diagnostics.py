...
OUTCOME_FEATURES = ['total_payement', 'received_principal', 'interest_received',
...
print(f"train & hold_out overlap : {len(tr_u & ho_u):>6,} users "
...
print(f"rows with number_of_loans == 0 : {len(zero_loans):,} "
      f"({100*len(zero_loans)/len(df):.2f}% of data)")
print(f"  of those, rows with total_payement > 0     : "
      f"{(zero_loans.total_payement > 0).sum():,}")
print(f"  of those, rows with received_principal > 0 : "
      f"{(zero_loans.received_principal > 0).sum():,}")
print(f"  their mean total_payement                 : "
      f"{zero_loans.total_payement.mean():,.2f}")
...
num_cols = ['total_income', 'dependents', 'delinq_2yrs', 'total_payement',
            'received_principal', 'interest_received', 'number_of_loans',
            'interest_received_ratio', 'total_payement_per_loan', 'delinq_2yrs_ratio']
rows = []
for c in num_cols:
...
for c in cat_cols:
    print(f"  {c:<22} {train[c].nunique():>7,} distinct   "
...
for c in cat_cols:
    v = enc_train[c]
    print(f"  {c:<22} min={v.min():.6f}  max={v.max():.6f}  std={v.std():.6f}  "
          f"nunique={v.nunique()}")
...
for c in ['interest_received_ratio', 'total_payement_per_loan', 'delinq_2yrs_ratio']:
    v = train[c]
    print(f"{c:<26} zeros={100*(v == 0).mean():>6.2f}%  std={v.std():.4f}  "
          f"nunique={v.nunique():,}")
...
def fit_and_score(feature_cols, tag):
    p = {'objective': 'binary', 'metric': 'auc', 'boosting': 'gbdt',
         'num_leaves': 12, 'max_depth': 9, 'learning_rate': 0.01,
         'feature_fraction': 0.669, 'max_bin': 100, 'min_data_in_leaf': 625,
         'min_data_in_bin': 30, 'bagging_freq': 20, 'random_seed': 2019,
    ...
    m = lgb.train(p, dtr, 20000, valid_sets=[dva], valid_names=['val'],
...
print(f"\nremoved as outcome-contaminated: "
      f"{[c for c in all_feats if c in OUTCOME_FEATURES]}")
