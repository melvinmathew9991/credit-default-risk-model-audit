...
from ml_pipeline.validation import (drop_leaked_users, expanding_window_folds,
                                    summarise_folds)
...
ENCODER_PARAMS = {"verbose": 0, "cols": None, "drop_invariant": False,
                  "return_df": True, "handle_missing": "value",
                  "handle_unknown": "value", "min_samples_leaf": 1000,
                  "smoothing": 1}
...
def parse_args(argv=None):
    ...
    p.add_argument("--no-restricted", action="store_true",
...
def build_datasets(cfg, args):
    """Load, label, clean, and split into tuning months and hold-out."""
...
def cv_score(cfg, tuning, features, lgb_params, num_boost_round, early_stopping):
    ...
    for _, train_months, val_month, tr_idx, va_idx in expanding_window_folds(
...
def main(argv=None):
    ...
    cfg = Config(data_path=args.data_path or Config().data_path,
                 output_dir=args.output_dir, max_evals=args.max_evals,
    ...
    logger.info("input fingerprint sha256=%s (%d bytes)",
                lineage["sha256"][:16] + "...", lineage["bytes"])

    df, tuning, hold_out = build_datasets(cfg, args)

    # ---- feature policy -------------------------------------------------
    features = governance.permitted_features(
        df.columns, allow_restricted=not args.no_restricted)
    governance.enforce_policy(features)
    logger.info("governance-permitted features (%d): %s", len(features), sorted(features))
    excluded = {c: governance.classify(c).value for c in df.columns
                if governance.classify(c) and c not in features
    ...
    def objective(space):
        params = {
            ...
            'lambda_l1': space['lambda_l1'], 'lambda_l2': space['lambda_l2'],
            'bagging_freq': 20,
        ...
        val_aucs, train_aucs, iters = cv_score(
            cfg, tuning, features, params, cfg.num_boost_round,
        ...
        state["i"] += 1
        row = {**{f"p_{k}": val for k, val in params.items()
                  if k not in ("objective", "metric", "boosting", "verbose")},
               "cv_val_auc_mean": v["mean"], "cv_val_auc_std": v["std"],
               "cv_val_auc_min": v["min"], "cv_train_auc_mean": t["mean"],
               "gap": gap, "mean_best_iter": float(np.mean(iters)), "score": score}
        trial_rows.append(row)
        pd.DataFrame(trial_rows).to_csv(cfg.path("hyperopt_results_v2.csv"), index=False)
        logger.info("trial %2d/%d  cv_val_auc=%.4f (+/-%.4f, min %.4f)  gap=%.4f  score=%.5f",
                    state["i"], cfg.max_evals, v["mean"], v["std"], v["min"], gap, score)
        return score

    logger.info("tuning: %d trials over walk-forward folds", cfg.max_evals)
    fmin(fn=objective, space=training.space, algo=tpe.suggest,
         max_evals=cfg.max_evals, trials=Trials(),
         rstate=np.random.default_rng(cfg.seed), show_progressbar=False)
    ...
    for k in ('num_leaves', 'max_depth', 'max_bin', 'min_data_in_leaf',
              'min_data_in_bin', 'bagging_freq', 'random_seed'):
        best_params[k] = int(best_params[k])
    best_params.update({'objective': 'binary', 'metric': 'auc',
                        'boosting': 'gbdt', 'verbose': -1})
    n_rounds = max(50, int(round(best["mean_best_iter"])))
    logger.info("best trial: cv_val_auc=%.4f (+/-%.4f), %d rounds",
                best["cv_val_auc_mean"], best["cv_val_auc_std"], n_rounds)
    ...
    model = lgb.train(best_params, lgb.Dataset(fit_e[features], label=fit_e.label),
                      num_boost_round=n_rounds)

    calibrator = Calibrator(args.calibration).fit(
        model.predict(calib_e[features]), calib_e.label)
    ...
    for name, obj in [("target_encoder_v2.pkl", encoder),
    ...
    print("=" * 86)
    print(f"CV val AUC across folds : {best['cv_val_auc_mean']:.4f} "
          f"+/- {best['cv_val_auc_std']:.4f} (min {best['cv_val_auc_min']:.4f})")
    print(f"\nCalibration ({args.calibration}):")
    b, a = calib["before"], calib["after"]
    print(f"  Brier {b['brier']:.5f} -> {a['brier']:.5f}   "
          f"ECE {b['expected_calibration_error']:.5f} -> {a['expected_calibration_error']:.5f}")
    print(f"  mean predicted {b['mean_predicted']:.4f} -> {a['mean_predicted']:.4f} "
          f"vs observed {a['observed_rate']:.4f}")
    print(f"  AUC {b['auc']:.4f} -> {a['auc']:.4f} ({calib['auc_delta']:+.4f}; "
