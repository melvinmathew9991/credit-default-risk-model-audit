...
from ml_pipeline import processing, training, utils
...
def build_objective(cfg, train, val, hold_out, state):
    ...
    def objective(space):
        ...
        state["i"] += 1
        logger.info(
            "trial %2d/%d  train_auc=%.4f val_auc=%.4f hold_out_auc=%.4f " "n_est=%d score=%.5f",
            state["i"],
            cfg.max_evals,
            train_auc,
            val_auc,
...
def main(argv=None):
    ...
    # 0. validate the input against the data contract
    # Report-only here: this entry point exists to reproduce the original model,
    # and the shipped dataset is known to violate the contract (see
    # MODEL_REVIEW.md H1). The point is that every run now records the state of
    # its input rather than discovering it two years later.
    validate_raw_data(pd.read_csv(cfg.data_path, low_memory=False), strict=False)
    ...
    logger.info(
        "best trial %d (score %.5f): %s",
        best_param_index,
        hyperopt_results.loc[best_param_index, "score"],
        lgbm_params,
    )

    # 9. train the final model
    clf = training.train_lgb(
        train,
        val,
        lgbm_params,
        num_boost_round=cfg.num_boost_round,
    ...
    logger.info(
        "artefacts written to %s/: model.txt, target_encoder.pkl, "
