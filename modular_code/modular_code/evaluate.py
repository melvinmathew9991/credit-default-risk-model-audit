...
# ---------------------------------------------------------------- metrics
report = {"n_features": len(clf.feature_name()),
...
fi = pd.DataFrame({"feature": clf.feature_name(),
                   "split": clf.feature_importance("split"),
                   "gain": clf.feature_importance("gain")}).sort_values("gain", ascending=False)
...
plt.savefig(os.path.join(PLOTS, "roc_curve.png"), dpi=120, bbox_inches="tight"); plt.close()

plt.figure(figsize=(11, 7))
for name in datasets:
    pr, re, _ = precision_recall_curve(labels[name], preds[name])
    plt.plot(re, pr, color=COLORS[name],
             label=f"{name} AP = {average_precision_score(labels[name], preds[name]):.4f}")
plt.xlabel("Recall"); plt.ylabel("Precision")
plt.title("Precision-Recall Curve (defaulter class)"); plt.legend(); plt.grid(alpha=.3)
plt.savefig(os.path.join(PLOTS, "pr_curve.png"), dpi=120, bbox_inches="tight"); plt.close()
...
for ax, name in zip(axes, datasets):
    ...
    ax.set_title(f"{name} - predicted score"); ax.set_xlabel("P(default)"); ax.legend()
plt.savefig(os.path.join(PLOTS, "score_distribution.png"), dpi=120, bbox_inches="tight"); plt.close()

plt.figure(figsize=(11, 7))
for name in datasets:
    d = ev.decile_table(labels[name], preds[name])
    plt.plot(d["decile"], d["bad_rate"], marker="o", color=COLORS[name], label=name)
plt.xlabel("Risk decile (1 = riskiest)"); plt.ylabel("Observed default rate")
plt.title("Class rate by score decile"); plt.legend(); plt.grid(alpha=.3)
plt.savefig(os.path.join(PLOTS, "class_rate.png"), dpi=120, bbox_inches="tight"); plt.close()
...
lim = 1.0
plt.plot([0, lim], [0, lim], "k--", lw=1, label="perfectly calibrated")
plt.xlabel("Mean predicted P(default)"); plt.ylabel("Observed default rate")
plt.title("Calibration"); plt.legend(); plt.grid(alpha=.3)
plt.savefig(os.path.join(PLOTS, "calibration.png"), dpi=120, bbox_inches="tight"); plt.close()

plt.figure(figsize=(10, 6))
top = fi.head(15).iloc[::-1]
plt.barh(top["feature"], top["gain_pct"], color="#4269d0")
plt.xlabel("% of total gain"); plt.title("LightGBM feature importance (gain)")
plt.savefig(os.path.join(PLOTS, "feature_importance.png"), dpi=120, bbox_inches="tight"); plt.close()
...
try:
    ...
    plt.savefig(os.path.join(PLOTS, "shap_summary.png"), dpi=120, bbox_inches="tight"); plt.close()
...
for name in datasets:
    ...
    print(f"{name:<10}{s['n']:>8}{100*s['default_rate']:>7.2f}%{s['roc_auc']:>9.4f}"
          f"{s['gini']:>8.4f}{s['ks']:>8.4f}{s['pr_auc_class1']:>9.4f}{s['brier']:>9.4f}")
print("=" * 78)
print("PSI  train->val      : %.4f" % report["stability"]["psi_train_vs_val"])
print("PSI  train->hold_out : %.4f" % report["stability"]["psi_train_vs_hold_out"])
